...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $can_crud) {
    ...
    if (isset($_POST['tambah'])) {
        ...
        if ($cek->num_rows > 0) {
            $_SESSION['msg'] = 'Pegawai sudah menjadi pejabat!';
        } else {
            $conn->query("INSERT INTO pejabat_penilai (pegawai_id, keterangan) VALUES ($pid, '$ket')");
    ...
    header('Location: kinerja_pejabat.php');
    exit();
}
                                                ...
                                                <div class="modal-content">
                                                    <form method="POST">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title">Edit Pejabat</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                        </div>
                    ...
                    <div class="modal fade" id="add" tabindex="-1">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                <form method="POST">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Tambah Pejabat Penilai</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                    </div>
                                    <div class="modal-body">
                                        <input type="hidden" name="tambah" value="1">
                                        <div class="mb-3"><label class="form-label">Pegawai</label><select name="pegawai_id" class="form-select" required>
                                                <option value="">-- Pilih --</option><?php foreach ($peg_list as $pg): ?><option value="<?= $pg['id'] ?>"><?= htmlspecialchars($pg['nama']) ?> (<?= htmlspecialchars($pg['jabatan']) ?>)</option><?php endforeach; ?>
                                            </select></div>
