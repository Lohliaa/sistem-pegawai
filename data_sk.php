...
if (isset($_GET['hapus']) && is_numeric($_GET['hapus'])) {
    ...
    header("Location: data_sk.php?msg=" . urlencode("Data berhasil dihapus")); exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    header("Location: data_sk.php?msg=" . urlencode($msg)); exit;
}
...
<style>
*{box-sizing:border-box;}body{background:#f4f6f9;font-family:'Segoe UI',Arial,sans-serif;font-size:13px;margin:0;padding:0;}
...
.main{background:#f4f6f9;min-height:100vh;padding:20px;}
@media(max-width:768px){.sidebar a span{display:none;}.sidebar a{text-align:center;padding:12px;}}
.card{border:none;border-radius:8px;box-shadow:0 1px 3px rgba(0,0,0,.1);margin-bottom:20px;}
.card-header{background:#fff;border-bottom:1px solid #e9ecef;padding:12px 20px;font-weight:600;color:#2c3e50;}
.card-body{padding:20px;}
.form-label{font-size:11px;font-weight:600;color:#555;margin-bottom:3px;}
.form-control,.form-select{font-size:12px;border-radius:6px;}
.btn{font-size:12px;border-radius:6px;}
#modalForm .modal-body{max-height:65vh;overflow-y:auto;padding:15px 20px;}
#modalForm .modal-footer{position:sticky;bottom:0;background:#f8f9fa;border-top:1px solid #dee2e6;padding:12px 20px;}
        #tblSK{font-size:11px;table-layout:fixed;width:100%;border-collapse:collapse;border-spacing:0;}
        #tblSK thead th{font-size:10px;font-weight:600;text-transform:uppercase;white-space:nowrap;padding:7px 6px;text-align:center;vertical-align:middle;border:1px solid #2c3e50;background:#2c3e50;color:#fff;box-sizing:border-box;}
        #tblSK tbody td{padding:5px 6px;vertical-align:middle;line-height:1.3;border:1px solid #dee2e6;font-size:11px;box-sizing:border-box;}
        #tblSK tbody tr:nth-child(odd)>td{background:#fafbfc;}
        #tblSK tbody tr:nth-child(even)>td{background:#ffffff;}
        #tblSK tbody tr:hover>td{background:#fff8e1 !important;}
        #tblSK .aksi-btns a{margin:0 1px;display:inline-block;}
        /* Pastikan garis header & isi lurus */
        #tblSK th,#tblSK td{box-sizing:border-box;}
...
<th style="width:60px;text-align:center;">Gol/Ruang</th>
<th style="width:100px;text-align:center;">Status Kepegawaian</th>
<th style="width:100px;">Unit Kerja</th>
<th style="width:75px;">TMT</th>
<th style="width:85px;">Tanggal Mulai</th>
<th style="width:75px;">Berlaku</th>
<th style="width:85px;">Tanggal Akhir</th>
<th style="width:85px;">Tanggal Ditetapkan</th>
...
<td style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="<?= htmlspecialchars($row['no_sk']) ?>"><?= htmlspecialchars($row['no_sk']) ?></td>
<td style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="<?= htmlspecialchars($row['no_tambahan']) ?>"><?= htmlspecialchars($row['no_tambahan']) ?></td>
<td style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="<?= htmlspecialchars($row['nama']) ?>"><?= htmlspecialchars($row['nama']) ?></td>
<td style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?= htmlspecialchars($row['gelar']) ?></td>
<td style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:center;" title="<?= htmlspecialchars($row['nipy']) ?>"><?= htmlspecialchars($row['nipy']) ?></td>
<td style="text-align:center;"><?= htmlspecialchars($row['gol_ruang']) ?></td>
<td style="text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="<?= htmlspecialchars($row['status_kepegawaian']) ?>"><?= htmlspecialchars($row['status_kepegawaian']) ?></td>
<td style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="<?= htmlspecialchars($row['unit_kerja']) ?>"><?= htmlspecialchars($row['unit_kerja']) ?></td>
<td style="text-align:center;"><?= htmlspecialchars($row['tmt']) ?></td>
<td style="text-align:center;"><?= htmlspecialchars($row['tanggal_mulai']) ?></td>
<td style="text-align:center;"><?= htmlspecialchars($row['berlaku']) ?></td>
<td style="text-align:center;"><?= htmlspecialchars($row['tanggal_akhir']) ?></td>
<td style="text-align:center;"><?= htmlspecialchars($row['tanggal_ditetapkan']) ?></td>
<td style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?= htmlspecialchars($row['tempat_lahir']) ?></td>
<td style="text-align:center;"><?= htmlspecialchars($row['tanggal_lahir']) ?></td>
<td style="text-align:center;white-space:nowrap;" class="aksi-btns">
...
<script>
$(function() {
$('#tblSK').DataTable({language:{url:'//cdn.datatables.net/plug-ins/1.11.5/i18n/id.json'},pageLength:10,scrollX:true,order:[],columnDefs:[{targets:'_all',orderable:true}]});
var modal = new bootstrap.Modal(document.getElementById('modalForm'));
$('#btnTambah').click(function(){$('#frmSK')[0].reset();$('#f_id').val('');$('#modalTitle').html('<i class="bi bi-plus-circle"></i> Tambah Data SK');modal.show();});
...
fields.forEach(function(k){var el=document.getElementById('f_'+k);if(el)el.value=(d[k]||'');});
modal.show();
},'json');
};
