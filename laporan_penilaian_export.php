...
$is_admin=($_SESSION['role']=='admin');$uid=$_SESSION['user_id'];
...
$f_pid=(int)($_GET['periode_id']??0);$f_jid=(int)($_GET['pejabat_id']??0);$f_eid=(int)($_GET['pegawai_id']??0);$sng=$_GET['single']??'';
if(!$is_admin){$f_eid=$pg['id']??0;}
$wh=[];if($f_eid>0)$wh[]="fp.pegawai_id=$f_eid";if($f_pid>0)$wh[]="fp.periode_id=$f_pid";if($f_jid>0)$wh[]="fp.pejabat_id=$f_jid";
if($sng){$p=explode('_',$sng);if(count($p)>=2){$wh[]="fp.pegawai_id=".(int)$p[0];$wh[]="fp.periode_id=".(int)$p[1];}}
$ws=count($wh)>0?"WHERE ".implode(" AND ",$wh):"";
$lpr=[];$sq="SELECT fp.*,p.nama np,p.jabatan jp,p.status_kepegawaian sk,pj.nama pn,pj.jabatan pj,pr.nama_kuartal nk,pr.periode_bulan pb,pr.tahun th FROM form_penilaian fp LEFT JOIN pegawai p ON fp.pegawai_id=p.id LEFT JOIN pejabat_penilai pj ON fp.pejabat_id=pj.id LEFT JOIN periode_penilaian pr ON fp.periode_id=pr.id $ws ORDER BY fp.pegawai_id,pr.tahun DESC";
$rs=$conn->query($sq);if($rs){while($r=$rs->fetch_assoc()){$k=$r['pegawai_id'].'_'.$r['periode_id'];$lpr[$k]['i']=['eid'=>$r['pegawai_id'],'np'=>$r['np'],'jp'=>$r['jp'],'sk'=>$r['sk'],'pn'=>$r['pn'],'pj'=>$r['pj'],'nk'=>$r['nk'],'pb'=>$r['pb'],'th'=>$r['th']];$lpr[$k]['n'][$r['nama_kolom']]=$r;}}
$act=$_GET['action']??'excel';
if($act=='excel'){
header('Content-Type:application/vnd.ms-excel');header('Content-Disposition:attachment;filename="laporan_penilaian_'.date('Ymd').'.xls"');
echo'<?xml version="1.0"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"><Worksheet ss:Name="Laporan"><Table>';
foreach($lpr as $lp){$i=$lp['i'];$tot=0;$jml=0;foreach($lp['n'] as $n){if($n['nilai']>0){$tot+=$n['nilai'];$jml++;}}$rt=$jml>0?round($tot/$jml,2):0;
...
foreach($uraian as $h=>$a){echo'<Row><Cell><Data ss:Type="String">'.$h.'. '.$a[0].'</Data></Cell></Row>';foreach($a[1] as $n=>$it){$p=explode('|',$it);$k=$p[0];$l=$p[1];$v=$lp['n'][$k]['nilai']??'';$c=$lp['n'][$k]['catatan']??'';echo'<Row><Cell><Data ss:Type="Number">'.$n.'</Data></Cell><Cell><Data ss:Type="String">'.htmlspecialchars($l ?? '').'</Data></Cell><Cell><Data ss:Type="Number">'.($v?:0).'</Data></Cell><Cell><Data ss:Type="String">'.htmlspecialchars($c ?? '').'</Data></Cell></Row>';}}
echo'<Row><Cell><Data ss:Type="String">TOTAL & RATA-RATA</Data></Cell><Cell><Data ss:Type="String">'.$tot.' / '.$rt.'</Data></Cell></Row><Row><Cell><Data ss:Type="String"></Data></Cell></Row>';}
...
foreach($uraian as$h=>$a){echo'<tr><td colspan="4" class="aspek">'.$h.'. '.$a[0].'</td></tr>';foreach($a[1] as$n=>$it){$p=explode('|',$it);$k=$p[0];$l=$p[1];$v=$lp['n'][$k]['nilai']??'';$c=$lp['n'][$k]['catatan']??'';echo'<tr><td style="text-align:center;">'.$n.'</td><td>'.htmlspecialchars($l ?? '').'</td><td style="text-align:center;">'.($v?:'-').'</td><td>'.htmlspecialchars($c ?? '').'</td></tr>';}}
...
echo'</body></html>';exit();}
header('Location:laporan_penilaian.php');exit();
